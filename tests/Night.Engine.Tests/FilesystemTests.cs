...
public class FilesystemTests : IDisposable
{
    ...
    public FilesystemTests()
    {
        ...
        Directory.CreateDirectory(testDirFullPath);
        ...
        try
        {
            File.CreateSymbolicLink(_testSymlinkFilePath, _testFilePath);
        }
        ...
        try
        {
             Directory.CreateSymbolicLink(_testSymlinkDirPath, _testDirPath);
        }
        catch (IOException ex) // More specific exception for symlink creation issues
        {
    ...
    public void ReadBytes_NonExistentFile_ThrowsFileNotFound()
    {
        var executionPath = Path.GetDirectoryName(typeof(FilesystemTests).Assembly.Location);
        var testDirFullPath = Path.Combine(executionPath!, TestDir);
        Assert.Throws<FileNotFoundException>(() => Night.Filesystem.ReadBytes(Path.Combine(testDirFullPath, "non_existent_file_for_read.txt")));
    }
    ...
    public void ReadText_NonExistentFile_ThrowsFileNotFound()
    {
        var executionPath = Path.GetDirectoryName(typeof(FilesystemTests).Assembly.Location);
        var testDirFullPath = Path.Combine(executionPath!, TestDir);
        Assert.Throws<FileNotFoundException>(() => Night.Filesystem.ReadText(Path.Combine(testDirFullPath, "non_existent_file_for_read.txt")));
    }
} 